...
class Inscription extends Model
{
    ...
    public function scopeWithTrashedRelations($query)
    {
        return $query->withTrashed()->with([
            ...
            'assistance' => fn($query) => $query->orderBy('month')->withTrashed(),
    ...
    public function getFormatAverageAttribute(): array
    {
        return [
            ...
            'played_approx_avg' => round(($this->skillsControls->avg('played_approx') ?? 0), 1),
            'goals' => $this->skillsControls->sum('goals'),
            'goals_avg' => round(($this->skillsControls->avg('goals') ?? 0), 1),
            'red_cards' => $this->skillsControls->sum('red_cards'),
            'red_cards_avg' => round(($this->skillsControls->avg('red_cards') ?? 0), 1),
            'yellow_cards' => $this->skillsControls->sum('yellow_cards'),
            'yellow_cards_avg' => round(($this->skillsControls->avg('yellow_cards') ?? 0), 1),
            'qualification' => round(($this->skillsControls->avg('qualification') ?? 0), 1),
