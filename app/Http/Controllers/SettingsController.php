...
class SettingsController extends Controller
{
    ...
    private function rememberConfigOptions(string $cacheKey, $ttl, string $configKey): array
    {
        return Cache::remember($cacheKey, $ttl, fn () => collect(config("variables.{$configKey}", []))
            ->map(fn ($label, $value) => [
                'value' => (string) $value,
                'label' => (string) $label,
            ])
    ...
    public function index()
    {
        ...
        $categories = Cache::remember(
            ...
            now()->addMinutes(5),
            fn () => DB::table('inscriptions')->where('school_id', $school_id)->where('year', now()->year)->orderBy('category')->groupBy('category')->select(['category'])->get()
        );

        $genders = $this->rememberConfigOptions('KEY_GENDERS', now()->addYear(), 'KEY_GENDERS');

        $positions = Cache::remember('KEY_POSITIONS', now()->addYear(), fn () => collect(config('variables.KEY_POSITIONS'))
            ->values()
            ->map(fn ($item) => ['id' => $item, 'name' => $item])
        );

        $blood_types = $this->rememberConfigOptions('KEY_BLOOD_TYPES', now()->addYear(), 'KEY_BLOOD_TYPES');

        $averages = $this->rememberConfigOptions('KEY_AVERAGES', now()->addYear(), 'KEY_AVERAGES');

        $dominant_profile = $this->rememberConfigOptions('KEY_DOMINANT_PROFILE', now()->addYear(), 'KEY_DOMINANT_PROFILE');

        $relationships = $this->rememberConfigOptions('KEY_RELATIONSHIPS_SELECT', now()->addYear(), 'KEY_RELATIONSHIPS_SELECT');
        ...
        $inscription_years = Cache::remember(
            ...
            now()->addMinutes(5),
            fn () => DB::table('inscriptions')->where('school_id', $school_id)->distinct('year')->orderBy('year')->select(['year as id', 'year'])->get()
        );

        $document_types = $this->rememberConfigOptions('KEY_DOCUMENT_TYPES', now()->addYear(), 'KEY_DOCUMENT_TYPES');

        $jornada = $this->rememberConfigOptions('KEY_JORNADA_TYPES', now()->addYear(), 'KEY_JORNADA');
