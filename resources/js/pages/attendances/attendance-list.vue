                                ...
                                <div class="row">
                                    <div class="col-md-12 col-sm-12 col-12 text-center mail-btn-container">
                                        <a data-bs-toggle="modal" data-bs-target="#composeMailModal"
                                            id="btn-compose-mail" class="btn btn-block" href="javascript:void(0);"
                                            @click="open_mail('add')">
                                    ...
                                    <div class="col-md-12 col-sm-12 col-12 mail-categories-container">
                                        <perfect-scrollbar class="mail-sidebar-scroll">
                                            <ul class="nav nav-pills d-block" id="pills-tab" role="tablist">
                                                ...
                                                <li class="nav-item" v-for="group in settings.groups" :key="group.id">
                                                    <a class="nav-link list-actions"
                                                        :class="{ active: selected_tab == 'inbox' }"
                                                        @click="tab_changed(group.id)">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                            ...
                            <div id="mailbox-inbox" class="accordion mailbox-inbox">
                                ...
                                <div v-show="!selected_mail" class="message-box">
                                    <perfect-scrollbar class="message-box-scroll" id="ct">
                                        <div v-for="(item, index) in filtered_mail_list" :key="item.id + '' + index"
                                            class="mail-item" :class="{ unread: item.is_unread }"
                                            @click="select_mail(item)">
                                                ...
                                                <div class="mb-0">
                                                    <div class="mail-item-heading">
                                                        <div class="mail-item-inner">
                                                            <div class="d-flex">
                                                                <div>
                                                                    <div class="checkbox-primary new-control me-2 custom-control custom-checkbox"
                                                                        @click.stop="$event.stopPropagation()">
                                                                        <input type="checkbox" :id="`chk-${item.id}`"
                                                                            v-model="ids" class="custom-control-input"
                                                                            :value="item.id" />
                                                                        <label class="custom-control-label"
                                                                            :for="`chk-${item.id}`"></label>
                                                                    </div>
                                                                </div>
                                                                <div class="f-head">
                                                                    <img v-if="item.path"
                                                                    ...
                                                                    <img v-else src="/assets/images/user-avtar.svg"
                                                                        class="user-profile" alt="avatar" />
                                                                </div>
                                                                <div class="f-body">
                                                                    <div class="meta-mail-time">
                                                                        <p v-if="item.first_name" class="user-email">{{
                                                                            item.first_name + " " +
                                                                            item.last_name }}</p>
                                                                        <p v-else class="user-email">{{ item.email }}
                                                                        </p>
                                                                    </div>
                                                                    <div class="meta-title-tag">
                                                                        <p class="mail-content-excerpt">
                                                                            <svg v-if="item.attachments && item.attachments.length"
                                                                        ...
                                                                        <div class="tags">
                                                                            <span v-if="item.group == 'personal'"
                                                                                class="g-dot-primary d-inline-block"></span>
                                                                            <span v-if="item.group == 'work'"
                                                                                class="g-dot-warning d-inline-block"></span>
                                                                            <span v-if="item.group == 'social'"
                                                                                class="g-dot-success d-inline-block"></span>
                                                                            <span v-if="item.group == 'private'"
                                                                                class="g-dot-danger d-inline-block"></span>
                                                                        </div>
                                                        ...
                                                        <div v-if="item.attachments && item.attachments.length"
                                                            class="attachments">
                                                            <span v-for="(files, ind) in item.attachments"
                                                                :key="'fl' + index + ind">{{ files.name
                                                                }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div v-if="selected_mail" class="content-box"
                                    :class="{ 'w-100 left-0': selected_mail }">
                                        ...
                                        <div class="mail-content-container">
                                            <div class="d-flex justify-content-between mb-3">
                                                <div class="d-flex user-info">
                                                    <div class="f-head">
                                                        <img v-if="selected_mail.path"
                                                            :src="`/assets/images/${selected_mail.path}`"
                                                            class="user-profile" alt="avatar" />
                                                        <div v-else-if="selected_mail.first_name"
                                                            class="avatar avatar-sm">
                                                            <span class="avatar-title rounded-circle">
                                                                {{ selected_mail.first_name.charAt(0) + "" +
                                                                selected_mail.last_name.charAt(0) }}
                                                            </span>
                                                        </div>
                                                        <img v-else src="/assets/images/user-avtar.svg"
                                                            class="user-profile" alt="avatar" />
                                                    ...
                                                    <div class="f-body">
                                                        <div class="meta-title-tag">
                                                            <h4 v-if="selected_mail.first_name" class="mail-usr-name">{{
                                                                selected_mail.first_name + " "
                                                                + selected_mail.last_name }}</h4>
                                                            <h4 v-else class="mail-usr-name">{{ selected_mail.email }}
                                                            </h4>
                                                        </div>
                                                        <div class="meta-mail-time">
                                                            <p class="user-email">{{ selected_mail.email }}</p>
                                                            <p class="mail-content-meta-date current-recent-mail ms-1">
                                                                {{ selected_mail.date }} -</p>
                                                            <p class="meta-time align-self-center ms-1">{{
                                                                selected_mail.time }}</p>
                                                        </div>
                                                    </div>
                                                </div>

                                                <!-- <div class="action-btns text-nowrap">
                                                    <a href="javascript:;" data-bs-toggle="tooltip" title="Reply"
                                                        @click="open_mail('reply', selected_mail)">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                            viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                            stroke-width="2" stroke-linecap="round"
                                                            stroke-linejoin="round"
                                                    ...
                                                    <a href="javascript:;" data-bs-toggle="tooltip" title="Forward"
                                                        @click="open_mail('forward', selected_mail)">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                            viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                            stroke-width="2" stroke-linecap="round"
                                                            stroke-linejoin="round"
