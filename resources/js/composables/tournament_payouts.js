...
export default function usePayouts() {
    ...
    const getPays = async ({competition_group_id, tournament_id, unique_code}) => {
        pays.value = []
        let response = await axios.get(`/api/tournamentpayout?tournament_id=${tournament_id}&competition_group_id=${competition_group_id}&unique_code=${unique_code}&dataRaw=true`)
